...
function Paging(sender) {
    ...
    AjaxFormSubmit({
        ...
        url: '/Admin/Categories/SubCategoryPaging',
...
function DeleteSubCat(sender) {
    ...
    if (checked.length <= 0) {
        Message('Please select at-least one sub category.', ActionStatus.Error);
        return false;
    }
    Message('Processing...');
    AjaxFormSubmit({
        ...
        messageControl: null,
        throbberPosition: { my: "left center", at: "right center", of: sender, offset: "5 0" },
        url: '/Admin/Categories/DeleteSubCat',
        success: function (data) {
            if (data.Status == ActionStatus.Successfull) {
                setTimeout(function () {
                    Message('Redirecting...', ActionStatus.Successfull);
